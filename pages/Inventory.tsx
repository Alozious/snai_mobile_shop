  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-black text-slate-800 tracking-tight">Product Inventory</h2>
          <p className="text-xs text-slate-500">Manage your shop's items and stock levels</p>
        </div>
                ...
                <tr key={p.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-brand-primary/10 flex items-center justify-center text-brand-primaryDark font-bold">
                        <Tag size={18} />
                      </div>
                      <div>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="text-sm font-bold text-slate-700">{(p.quantity ?? 0)} units</div>
                    </div>
                    <div className="text-[10px] text-slate-400">Min. req: {(p.reorderLevel ?? 0)}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm font-bold text-emerald-600">{(p.sellingPrice ?? 0).toLocaleString()}</div>
                    <div className="text-[10px] text-slate-400 line-through">{(p.costPrice ?? 0).toLocaleString()}</div>
                  </td>
                  <td className="px-6 py-4">
          ...
          <div className="bg-white w-full max-w-xl rounded-[2.5rem] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSave} className="p-8 pt-4 space-y-6">
              <div className="grid grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Selling Price (UGX)</label>
                  <input 
                    required 
                    type="number" 
                    placeholder="0"
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Reorder Level</label>
                  <input 
                    required 
                    type="number" 
                    placeholder="5"
              ...
              <div className="flex gap-4 pt-6">
                <button 
                  type="button"
                  onClick={() => setShowModal(false)}
                  className="flex-1 px-6 py-4 border border-slate-200 rounded-2xl text-xs font-black text-slate-500 uppercase tracking-[0.2em] hover:bg-slate-50 transition-all active:scale-95"
                >
                  Cancel
                </button>
                <button 
                  type="submit"
                  className="flex-1 px-6 py-4 bg-brand-primary text-black rounded-2xl text-xs font-black uppercase tracking-[0.2em] hover:bg-brand-primaryDark shadow-xl shadow-brand-primary/20 hover:shadow-brand-primary/30 transition-all active:scale-95"
                >
                  Save Product
                </button>
              </div>
